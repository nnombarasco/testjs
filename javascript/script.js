...
let showDetails_ = function(indice){
  for(let i = 0; i< allPlaces.length; i++){
    let array = allPlaces[i].places;
    for(let y = 0; y < array.length; y++){
      if(array[y].id == indice){
        console.log(array[y].id);//tenemos el dato
      }
    }
  }
  /*let div = document.createElement("div");
  div.setAttribute("id", "container");
  div.innerHTML+=`<div><h3></h3></div>`
...
let showPlaces = function(indice){
  ...
  for (const iterator of newPlaces) {
    cards.innerHTML += `
        ...
        <button onClick=showDetails_(${iterator.id})>Book Now</button>
        </div>
        `;
  }
